...
public:
    ...
    bool operator == (const foreign& other) const
    {
...
int main()
{
    ...
    HashMap<foreign, string> my_map(100000);
    ...
    while(getline(cin, str))
    {
        if(str.compare("") == 0)
            break;
        //cout << "X" << str.substr(0, str.find(' ')) << "X" << endl;
        //cout << "X" << str.substr(str.find(' ') + 1) << "X" << endl;
        my_map.insert(foreign(str.substr(str.find(' ') + 1)), str.substr(0, str.find(' ')));
    }
    ...
    while(getline(cin, str))
    {
        ...
        string result;
        if(!my_map.query(foreign(str), result))
            result = "eh";
        cout << result << endl;
    }
